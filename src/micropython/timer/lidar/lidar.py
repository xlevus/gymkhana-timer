...
class LIDAR:
    ...
    def _writebyte(self, addr, byte):
        self._write(addr, bytearray([byte]))

    def save(self):
        self._writebyte(const.SAVE, 0x01)
        utime.sleep_ms(100)

    def reboot(self):
        utime.sleep_ms(50)
        self.save()
        self._writebyte(const.SHUTDOWN_REBOOT, 0x02)
    ...
    def set_frequency(self, freq=0x64):
        self._writebyte(const.FPS_LOW, freq)
        self._save_reboot()

    def power_saving_mode(self, power_saving_mode=True):
        val = 0x01 if power_saving_mode else 0x00
        self._writebyte(const.LOW_POWER, val)

    def on_off(self, on=True):
        val = 0x00 if on else 0x01
        self._writebyte(const.ENABLE, val)

    def reset(self):
        self._writebyte(const.RESTORE_FACTORY_DEFAULTS, 0x01)
        self._save_reboot()
    ...
    def set_min_max(self, min, max):
        ...
        self._writebyte(const.MIN_DIST_HIGH, high)
        self._writebyte(const.MIN_DIST_LOW, low)

        high, low  = max >> 8, max & 0XFF
        self._writebyte(const.MAX_DIST_HIGH, high)
        self._writebyte(const.MAX_DIST_LOW, low)
        self._save_reboot()
